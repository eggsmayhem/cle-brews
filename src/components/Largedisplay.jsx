             ...
             <div className='flex flex-col w-2/4 gap-y-1.5 items-center text-center'>
                ...
                {displayNumber.length > 4 &&
                    <span className="flex gap-x-1.5 items-center font-semibold">
                        <img src={phone} className="h-[16px]"/><a href={`tel:${displayNumber}`} className="text-s">{displayNumber.substring(0,3)+ "-" + displayNumber.substring(3,6)+ "-"+displayNumber.substring(6,10)}</a>
                    </span> 
                }
            </div>
